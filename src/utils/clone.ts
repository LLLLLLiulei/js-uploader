...
export const clone = <T = unknown>(origin: T, target: T | null = null): T | null => {
  ...
  if (((origin as unknown) as Function).constructor === ArrayBuffer) {
    const buffer = (origin as unknown) as ArrayBuffer
    return buffer.slice(0, buffer.byteLength) as any
  }

  if (
    ((origin as unknown) as Function).constructor.name === 'Blob' ||
    ((origin as unknown) as Function).constructor.name === 'ArrayBuffer'
  ) {
    const data = origin as any
    const slice = data.slice || data.webkitSlice || data.mozSlice
    return (slice as Function).call(data, 0, data.length)
  }
